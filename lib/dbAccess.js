...
//    return CryptoJS.AES.encrypt(password, 'This should be random').toString();
  ...
  async createAgent(agentInfo) {
    ...
    rows = await this.getAgentWithId(id);
    return rows[0];
  }
  ...
  async createListing(listingInfo) {
    ...
    let query = 'INSERT INTO listings (poster_id, property_address, listing_status, estimated_value, transaction_type, type_of_home) VALUES (?, ?, ?, ?, ?)';
    let args = [
  ...
  async getListingsForUser(user_id, user_type) {
    if(user_type === 'agent') {
      ...
      let query = 'SELECT listings.* from (bids INNER JOIN listings ON bids.listing_id = listings.id) WHERE bids.bid_status="Signed" AND bids.agent_id=?';
      let args = [user_id];
      const rows = await this.db.query(query, args);
      return rows;
  ...
  async getBidsForAgentWithIdActiveOrRejected(agent_id) {
    ...
    let query = 'SELECT listings.*, bids.bid_status from (bids INNER JOIN listings ON bids.listing_id = listings.id) WHERE (bids.bid_status="Active" OR bids.bid_status="Rejected") AND bids.agent_id=?;';
    let args = [agent_id];
    const rows = await this.db.query(query, args);
    return rows;
  }
