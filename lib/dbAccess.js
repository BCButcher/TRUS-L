...
class DBAccess {
  ...
  async updateAgent(id, newAgent) {
    let query = 'UPDATE agents SET license=?, SET first_name=?, SET last_name=?, SET email=?, SET phone=?, SET web_site=? WHERE id=?';
    let args = [newAgent.license, newAgent.first_name, newAgent.last_name, newAgent.email, newAgent.phone, id];
    const rows = await this.db.query(query, args);
    return rows;
  }

  async updateConsumer(id, newConsumer) {
    let query = 'UPDATE consumers SET display_name=?, SET first_name=?, SET last_name=?, SET email=? WHERE id=?';
  ...
  async createConsumer(consumerInfo) {
    ...
    let query = 'INSERT INTO consumers (display_name, first_name, last_name, email) VALUES (?, ?, ?, ?)';
    let args = [
  ...
  async createAgent(agentInfo) {
    ...
    let query = 'INSERT INTO agents (license, first_name, last_name, email, phone, web_site) VALUES (?, ?, ?, ?, ?, ?)';
    let args = [
      agentInfo.license,
  ...
  async getListingBidsWithStatus(listingStatus) {
    // SELECT * from bids WHERE listing_status = ?
    let query = 'SELECT * from bids WHERE listing_status = ?';
    let args = [listingStatus];
    const rows = await this.db.query(query, args);
    return rows;
  }
...
async function main() {
  ...
  console.log('in main, number of rows returned is: ' + listings.length);
